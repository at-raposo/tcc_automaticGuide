int trigPin = 13; // pino do trigger(gatilho de disparo de pulso do sensor ultrassônico
int echoPin = 12; // pino do echo(pino de leitura do tempo de resposta) do sensor ultrassônico
...
void loop() {
  ...
  if (distance <= 5) { // se a distância for igual ou menor que 5
    digitalWrite(vibPin, vib5); //vibrar na frequência de 400
    delay(500); //espera 0,5segundos para detectar novamente a distância
